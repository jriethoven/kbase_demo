package jeanjack_ContigFilter::jeanjack_ContigFilterClient;
...
jeanjack_ContigFilter::jeanjack_ContigFilterClient
...
sub new
{
    ...
    my $self = {
	client => jeanjack_ContigFilter::jeanjack_ContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a jeanjack_ContigFilter.FilterContigsParams
$return is a jeanjack_ContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a jeanjack_ContigFilter.workspace_name
	contigset_id has a value which is a jeanjack_ContigFilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a jeanjack_ContigFilter.ws_contigset_id
...
# Authentication: required
	...
	method => "jeanjack_ContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
<pre>
$params is a jeanjack_ContigFilter.FilterContigsMaxParams
$return is a jeanjack_ContigFilter.FilterContigsResults
...
# Authentication: required
    ...
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
	method => "jeanjack_ContigFilter.filter_contigs_max",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub version {
    my ($self) = @_;
    my $result = $self->{client}->call($self->{url}, $self->{headers}, {
        method => "jeanjack_ContigFilter.version",
        params => [],
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for jeanjack_ContigFilter::jeanjack_ContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "jeanjack_ContigFilter::jeanjack_ContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package jeanjack_ContigFilter::jeanjack_ContigFilterClient::RpcClient;
